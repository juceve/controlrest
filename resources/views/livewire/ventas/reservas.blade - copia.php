<div>
    @section('template_title')
        Reservas
    @endsection
    @if (cajaCerrada(Auth::user()->id, Auth::user()->sucursale_id ? Auth::user()->sucursale_id : ''))
    ...
    <div class="card">
        <div class="card-body">
            <h4 class="header-title mb-3">RESERVA DE PRODUCTOS</h4>
            {{-- <form> --}}
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover table-striped" style="font-size: 12px;">
                                        ...
                                        @if (!is_null($eventos))
                                            @foreach ($eventos as $evento)
                                                @if (!esFeriado($evento->fecha))
                                                    <th>{{ nombreDiaESP($evento->fecha) .
                                                        "
                                                                                                                                                                                                            " .
                                                        date('d-', strtotime($evento->fecha)) .
                                                        nombreMesESP($evento->fecha) }}
                                                    </th>
                                                @endif
                                            @endforeach
                                        @endif
                                ...
                                <tbody id="tbodyProductos">
                                    <tr>
                                        <td align="center" colspan="{{ $eventos->count() + 1 }}">Seleccione Clientes!
                                        </td>
                                    </tr>
                                </tbody>
                            ...
                            <div class="col-12 col-md-6 border" style="font-size: 12px; vertical-align: middle">
                                <table class="table table-sm">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td><input type="text" readonly class="form-control bg-white"
                                                    wire:model='importeTotalFloat'></td>
                                        </tr>
                                    </tbody>
                                </table>
                                ...
                                <div class="finish text-center mb-3 d-grid gap-2">
                                    @if (!cajaCerrada(Auth::user()->id, Auth::user()->sucursale_id))
                                        <button class="btn btn-success" wire:click="registrarReserva"
                                            wire:loading.attr="disabled">Registrar Reserva <i
...
@section('js')
    ...
    <script>
        ...
        Livewire.on('htmlFooter', html => {
            $('#tbodyFooter').empty();
            $('#tbodyFooter').html(html);
        });

        function aplicaTodos(pos, val, evento_id, fecha) {
            $('.sl' + pos).val(val);
            Livewire.emit('selectProductoAll', evento_id, val, fecha);
        }

        function aplicaFoot(pos, evento_id, estudiante_id, menu_id, fecha) {
            $('.foot' + pos).val("");
            Livewire.emit('selectProducto', evento_id, estudiante_id, menu_id, fecha);
        }
        ...
        Livewire.on('imprimir', data => {
            if (browseMobile) {
                window.open("/impresiones/recibos/mobile/reservas.php?data=" + data, "_blank");
            } else {
                window.open("/impresiones/recibos/reservas.php?data=" + data, "_blank");
            }
